...
export const DEFAULT_EVIDENCE_MARKDOWN_PATH =
  "docs/production-scale/evidence/latest-production-scale-evidence.md";
export const DEFAULT_EVIDENCE_JSON_PATH =
...
export function detectProductionEnvironment(env = process.env) {
  for (const key of PRODUCTION_ENV_KEYS) {
    ...
    if (value === "production" || value === "prod" || value.includes("production")) {
      return {
        productionLike: true,
        reason: `${key} indicates a production environment.`,
      };
    }
  }
  ...
  for (const key of PRODUCTION_SECRET_KEYS) {
    ...
    if (!value) continue;
    if (
      value.includes("creditregulatorpro-prod") ||
      ...
      return {
        productionLike: true,
        reason: `${key} appears to reference a production database target.`,
      };
    }
  }
...
export function loadBlockerRegistry({
  rootDir = process.cwd(),
  registryPath = DEFAULT_BLOCKER_REGISTRY_PATH,
} = {}) {
  const registry = JSON.parse(readText(rootDir, registryPath));
  return {
    ...registry,
    registryPath,
  };
...
export function validateBlockerRegistry(registry, auditRows = []) {
  ...
  if (blockers.length !== expectedCount) {
    errors.push(`Expected ${expectedCount} blockers, found ${blockers.length}.`);
  }
  ...
  if (duplicateNumbers.length > 0) {
    errors.push(`Duplicate blocker number(s): ${Array.from(new Set(duplicateNumbers)).join(", ")}.`);
  }
  ...
  if (missingNumbers.length > 0) {
    errors.push(`Missing blocker number(s): ${missingNumbers.join(", ")}.`);
  }
  ...
  for (const row of auditRows) {
    ...
    if (blocker.severity !== row.severity) {
      errors.push(`Blocker ${row.number} severity mismatch: registry ${blocker.severity}, audit ${row.severity}.`);
    }
    if (blocker.area !== row.area) {
      errors.push(`Blocker ${row.number} area mismatch: registry ${blocker.area}, audit ${row.area}.`);
    }
  }
  ...
  for (const blocker of blockers) {
    ...
    if (!statusValues.has(blocker.currentStatus)) {
      errors.push(`Blocker ${blocker.number} has invalid status ${blocker.currentStatus}.`);
    }
    if (!Array.isArray(blocker.proofCategories) || blocker.proofCategories.length === 0) {
      ...
      for (const category of blocker.proofCategories) {
        if (!proofCategories.has(category)) {
          errors.push(`Blocker ${blocker.number} has invalid proof category ${category}.`);
        }
      }
    }
    ...
    if (!Array.isArray(blocker.allowedProofCommands)) {
      ...
      for (const command of blocker.allowedProofCommands) {
        if (!recognizedCommands.has(command)) {
          errors.push(`Blocker ${blocker.number} has unrecognized evidence command: ${command}.`);
        }
      }
    }
    ...
    if (blocker.productionMutationForbidden !== true) {
      errors.push(`Blocker ${blocker.number} must forbid production mutation.`);
    }
    ...
    if (
      ...
      (!Array.isArray(blocker.allowedProofCommands) ||
        !blocker.allowedProofCommands.some((command) => recognizedCommands.has(command)))
    ) {
...
export function collectDashboardEvidence({
  rootDir = process.cwd(),
...
export function buildProductionScaleEvidenceReport({
  ...
  for (const group of OUTPUT_GROUPS) {
    evidence[group.key] = {
      ...
      productionProof: group.category === "read-only-production" ? "human-required-read-only" : false,
  ...
  evidence.simulated.productionProof = false;
  ...
  return {
    ...
    registry: {
      ...
      allAuditBlockersRepresented: auditRows.length === loadedRegistry.expectedBlockerCount && unresolved.length + waived.length <= blockers.length,
...
export function renderProductionScaleEvidenceMarkdown(report) {
  ...
  const lines = [
    ...
    `All 25 blockers represented: ${report.registry.actualBlockerCount === 25 ? "yes" : "no"}`,
    ...
    `- Status counts: ${Object.entries(report.registry.statusCounts)
