...
6. [x] Add operator launch policy limiting beta concurrency, upload size, packet volume, and response-worker operations until ingest queue exists. Implemented in `docs/limited-beta-operator-launch-policy.md`, with repository-artifact visibility added to `scripts/production-readiness-report.ts` and `scripts/operator-regression-dashboard.ts`. Selected beta limits: maximum 5 beta users, 3 concurrent active users, 1 concurrent report upload/process operation, 25 total report uploads per day, 5 report uploads per user per day, 15 MiB authenticated report uploads, 20 MiB anonymous report uploads if enabled, 10 MiB evidence/bureau uploads, 25 packet creations per day, 50 packet PDF download/send operations per day, deterministic OCR only with operator review or hold/reject, response-worker dry-run before/after beta windows, and response-worker/replay/lifecycle apply or non-dry operation only in supervised windows capped at 10 jobs or records. Remaining blockers for broader production: durable ingest/OCR/compliance queue, externalized encrypted report storage and retention proof, queued/cached packet PDF generation, ingest/PDF/storage/auth/DB observability, completed production restore drill, production-scale smoke/load coverage, and external alert delivery or formal acceptance of its absence.
