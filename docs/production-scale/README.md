...
The gate policy is `docs/production-scale/migration-governance-policy.json`. `migrations:gate` is also static and non-mutating. It fails closed for unknown schema mutation sources, unledgered mutation sources, missing expected sources, missing inventory entries, unapproved bootstrap mutation sources, unauthorized runtime ensure paths, invalid converted migration entries, expired temporary allowlist entries, and forbidden mutation patterns. It does not connect to a database, run DDL, inspect production, or alter deployment state.

Current gate mode is `release-blocking`. The first converted runtime ensure source is `helpers/ingestProcessingQueueSchema.ts`, represented by `migrations/0001-ingest-processing-queue-reviewed-additive.sql`. Remaining runtime ensure residuals are temporary production allowlist entries with owner, reason, expiry, and `CERTIFYING:false`. The production promotion gate may pass with valid temporary allowlist entries, but migration governance is not production-certifying until those entries are converted or removed.
