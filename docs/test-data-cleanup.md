...
Use `pnpm cleanup:test-data` to remove old, explicitly marked test data from local or staging databases. The default age threshold is 10 days.
...
Rows must be older than 10 days and explicitly test-marked.
...
Markers include clear test/demo/seed/fixture/mock/synthetic/smoke/parser-lab/lifecycle-test/beta-test/development-only labels, `example.test`, `example.invalid`, `auth.workflow`, `ingest_queue_test`, response queue/soak/orchestration prefixes, and `OUTCOME_SMOKE`.
...
- Response queue, response soak, and worker orchestration smokes: old explicitly marked jobs/runs/events are eligible.
