...
- Staging ingest worker operation is explicitly bounded and staging-only. Operators can preview with `pnpm run staging:ingest-worker -- --dry-run` and can run a one-shot apply pass with `pnpm run staging:ingest-worker -- --apply --max-jobs 5 --concurrency 1 --worker-id staging-ingest-manual`; the manual staging deploy workflow input `run_ingest_worker=true` runs the same bounded apply path after staging health and response-auth smokes. Production activation remains deferred and is not part of this checklist.
