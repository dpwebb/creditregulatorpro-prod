...
- Response processing queue coverage passes: `pnpm exec vitest run tests/api/response-processing-queue.spec.ts`, and the bounded worker dry-run command succeeds: `pnpm run response:worker -- --dry-run`. Queue jobs must remain DB-backed, sanitized, idempotent for active duplicate keys, row-lock claimed, retry/dead-letter deterministic, and append-only through `response_processing_job_event`; worker processing must not store raw response text or mutate canonical report facts, tradeline facts, violation truth, packet eligibility, or readiness rules.
