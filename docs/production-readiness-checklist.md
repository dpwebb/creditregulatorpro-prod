...
- Consumer-facing outcome UI, response capture UI, response parser/inbox integration, authenticated response admin-review UI smoke, historical backfill, or packet outcome automation.
...
- Authenticated response admin-review UI smoke remains pending after the first UI control deployment; run `pnpm run smoke:response-document-admin-review-ui` with `CRP_RESPONSE_DOCUMENT_ADMIN_REVIEW_UI_SMOKE=true`, safe admin auth, and verified synthetic response data before relying on the new metadata review controls for release.
