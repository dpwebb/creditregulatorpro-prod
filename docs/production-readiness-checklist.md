...
- Full consumer-facing outcome automation, live mailbox/inbox integration, historical backfill, or packet outcome automation.
...
- Response replay/backfill coverage passes inside `tests/api/response-document-endpoint.spec.ts`, and the local operator dry-run command succeeds: `pnpm run response:replay -- --dry-run`. Replay must remain dry-run by default; apply mode must require `--apply --confirm-apply --actor-user-id <id>` and write append-only processing/audit events only.
