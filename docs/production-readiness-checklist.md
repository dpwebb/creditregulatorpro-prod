...
- Response classification engine coverage passes: `pnpm exec vitest run tests/unit/response-classification-engine.spec.ts`, including deterministic classification, confidence gating, negation and contradiction handling, metadata-only/OCR-damaged fail-closed behavior, evidence-linked provenance, manual-review fail-closed states, regulation-reference review links, and no readiness/violation truth mutation.
