...
| Retention dry-run/apply guard | Complete with simulated lifecycle proof | Preview default and `APPLY_RETENTION_PURGE` apply confirmation; tracker row 12 and `latest-retention-archive-restore-simulated` evidence. | Retention/API tests and SIMULATED archive/restore proof passed. | Physical archive/restore proof remains incomplete and requires human-observed evidence. | Closed for apply guard; partial for recoverability. |
...
| Storage/retention/privacy | Partial | New report PDF references, metadata-only lists, retention apply guard passed, and SIMULATED retention archive/restore evidence exists. | Complete storage lifecycle, historical inline plan, and human-observed physical purge/archive/restore proof. | production |
...
| Cron/retention/scheduled jobs | Partial | Bearer cron, retention confirmation tests, and SIMULATED retention archive/restore proof passed. | Collect scheduled-operation evidence and human-observed physical retention archive/restore evidence. | scale |
