# Ingestion Compliance Finding Coverage

Verified: May 8, 2026

## Terminology

Use **compliance finding** as the umbrella product term. In code and database identifiers, the legacy `violation` naming remains for compatibility, including `violationCategory` and violation-correction endpoints.

A compliance finding is a detected credit-report issue tied to source evidence, a detector category, and a mapped rule or authority reference. It is not always a confirmed legal violation. Reserve **confirmed legal violation** for findings whose authority classification explicitly supports that label.

## Current Coverage Summary

For each ingested tradeline, the compliance scanner currently checks:

- 50 enabled finding categories with bona fide local authority mappings.
...
The platform currently has 50 configured finding categories. All 50 are enabled and all 50 resolve to at least one bona fide local authority mapping through the local registry.
...
That matters for missing-information issues. Missing information that supports a review status is only an actionable compliance finding when it can be mapped to a specific local or federal law, regulation, or official/private reporting standard that requires that exact field for the account type.
...
The evidence builder fails closed for exact-field authority. A field-specific authority record is not attached unless the finding supplies the matching field name, account or record type, consumer province, and finding category. Unknown province or missing account type prevents exact-field authority from being borrowed.
...
> Each ingestion checks 50 authority-backed compliance finding categories and approximately 70 concrete runtime checks per tradeline. However, missing-field issues remain review-only unless the local registry contains an exact field-level authority record for that field and account type.
