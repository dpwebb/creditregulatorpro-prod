...
| Response ops readiness pack | `pnpm run response-ops:readiness-evidence` | Operator-ready evidence command exists. | Live scheduler status, backfill readiness, purge/archive readiness, alerting status, dashboard SKIP count, response soak status, monitoring cadence, manual fallback, dashboard references, soak references, and unresolved risks. |
