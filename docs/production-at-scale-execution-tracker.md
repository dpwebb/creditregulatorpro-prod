...
| 2 | Phase 1 regression guard | Not started | Maximum audit says Phase 1 blockers were green at audited commit; this tracker does not rerun the full suite. | `pnpm run typecheck`; `pnpm run build`; `pnpm run test:contracts`; relevant Phase 1 API tests; `pnpm run test:golden-path`; `pnpm run test:regression-dashboard`; `pnpm run operator:dashboard` when runtime work begins. |
