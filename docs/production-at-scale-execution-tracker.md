...
- Retention auto-purge now defaults to preview and destructive apply requires explicit confirmation with append-only audit evidence. `pnpm run retention:archive-restore:simulated` now provides SIMULATED archive/restore lifecycle proof with synthetic records and audit markers, but physical retention archive/restore remains unresolved until human-observed evidence exists.
...
- Runtime-size reporting now tracks frontend bundle assets, gzip/brotli sizes, direct dependency installed sizes, PDF/OCR dependency inventory, and Docker OCR/runtime packages. `docs/production-scale/runtime-size-threshold-policy.json` classifies configured rows as `PASS`, `WARN`, `FAIL`, or `WAIVED`; the current policy is warning-only and writes `latest-runtime-size` evidence. Hard performance gates, chunking work, or dependency isolation remain future work before any production-at-scale claim.
...
| 12a | Retention archive/restore simulated proof | Partial | `scripts/retention-archive-restore-simulated.mjs` and `retention:archive-restore:simulated` create SIMULATED evidence for synthetic retention preview, archive marker/write, restore verification, synthetic audit events, apply-guard verification, production-environment refusal, and no-PII/no-secret validation. It writes `docs/production-scale/evidence/latest-retention-archive-restore-simulated.md` and `.json`. This is not physical archive/restore completion and is not disaster recovery proof. | `pnpm run retention:archive-restore:simulated`; `pnpm exec vitest run --config vitest.config.ts tests/unit/retention-archive-restore-simulated.spec.ts`; `pnpm exec vitest run --config vitest.config.ts tests/api/retention-apply-guard-endpoint.spec.ts`; `pnpm run check:restore-drill-evidence`; `git diff --check`. |
...
| 22 | Dependency/runtime report | Complete, warning-only policy | `scripts/runtime-size-report.mjs`, `report:runtime-size`, and `check:runtime-size` generate policy-bound runtime evidence for frontend build assets with gzip/brotli sizes, installed direct dependency sizes, PDF/OCR package usage, and Docker OCR/runtime packages. `docs/production-scale/runtime-size-threshold-policy.json` currently runs in warning-only mode: exceeded accepted thresholds are visible as `WARN`, Docker source-only runtime package sizing is visible as `WAIVED` with reason, and `FAIL` can occur only after an explicit future hard-gate policy change. Evidence is written to `docs/production-scale/evidence/latest-runtime-size.md` and `.json`. | `pnpm run build`; `pnpm run report:runtime-size`; `pnpm run check:runtime-size`; `pnpm exec vitest run --config vitest.config.ts tests/unit/runtime-size-report-script.spec.ts`; `git diff --check`. |
| 23 | Final production-at-scale verification | Complete | `docs/final-production-at-scale-verification.md` records the final conservative verdict for commit `e7ff406ebc86b852cf63b17845daecd854176f55`: limited beta ready with strict constraints, not broad-production ready, and not production-at-scale ready. All required local verification commands passed after an initial transient `operator:dashboard` process exit was rerun successfully; remaining blockers include no completed restore drill evidence, incomplete response production-operations proof, dry-run-only load evidence, warning-only runtime-size thresholds, and documented upload/storage residual risks. | `pnpm run typecheck`; `pnpm run build`; `pnpm run test:contracts`; `pnpm run test:api`; `pnpm run test:golden-path`; `pnpm run test:regression-dashboard`; `pnpm run test:deterministic-ingestion-report`; `pnpm run response:soak-check`; `pnpm run operator:dashboard`; `pnpm run check:migrations`; `pnpm run check:restore-drill-evidence`; `pnpm run baseline:production-scale-local -- --dry-run`; `pnpm run report:runtime-size`; `git diff --check`. |
