...
- Legacy and admin base64 surfaces remain insufficiently bounded outside report-artifact create/update, review approval, and consumer identification upload.
...
| 8 | Remaining upload/base64 boundary hardening | In progress | Report-artifact create/update, review approval, and consumer identification upload now use strict decoded-byte validation, MIME allowlists, raw request-size guards, and bounded filenames/metadata; review approval authenticates before body parsing. The workstream remains open because other parser/admin legacy upload surfaces and bureau communication DB-base64 storage remain separate unresolved tasks. | Oversize/malformed/MIME tests for each route; raw body guard tests; downstream-not-called tests where practical; valid current path regression. |
