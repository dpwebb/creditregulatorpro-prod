...
25. A gated advisory diagnostic endpoint smoke harness exists, but authenticated advisory diagnostic endpoint smoke remains pending until a safe staging admin context is available.
...
1. Run the gated advisory bridge diagnostic endpoint smoke harness with a safe staging admin context; after authenticated endpoint smoke passes, consider design-only admin UI advisory diagnostics only if operationally useful.
