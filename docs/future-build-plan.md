...
Status: Started. A deterministic no-schema comparison helper exists, persisted outcome comparison runs and finding outcomes exist, compare/list/get endpoints exist, authenticated staging smoke has passed for a synthetic response-only outcome, an admin-only metadata-review backend endpoint exists with authenticated staging smoke passed, the admin-only Outcome Reviews UI exists with authenticated staging smoke passed, and a metadata/evidence-only response-document capture backend now exists with deployed capture/list/get endpoints. Consumer-facing outcome UI, response-document UI, response parser, inbox integration, admin response-review endpoint, authenticated response capture smoke, historical backfill, production-scale repeated outcome smoke coverage, and monitoring remain future work.
...
9. Add admin/user response capture surfaces, admin response review, gated authenticated response-capture smoke, and monitoring only after the metadata-only backend boundary remains stable.
...
3. Add a gated response-document capture smoke harness and design the response-document UI/admin-review boundary for bureau communications, including Equifax Canada email responses; keep response documents as evidence only until deterministic parsing/comparison is separately implemented, and do not treat email response as corrected or removed without later report comparison.
4. Design historical backfill strategy and outcome monitoring/alerting before broader production-scale outcome usage; keep packet readiness and packet wording unchanged and do not add an admin override.
5. Add external alert-delivery validation for container health, HTTP 5xx spikes, parser/OCR failures, packet generation failures, outcome workflow failures, and background job/log error spikes if alerting infrastructure is available.
6. Run and record a human-observed staging restore drill into a local-only database using the existing refresh script before any broad production-scale launch.
7. Consider design-only admin UI advisory diagnostics only if operationally useful after the authenticated advisory diagnostic endpoint smoke pass.
8. Do not add a runtime selector yet.
9. Keep the DB regulation registry non-runtime until advisory or limited-runtime bridge rules, tests, rollback, and approval are implemented.
10. Keep packet wording and packet readiness unchanged as part of regulation/reference reconciliation and outcome tracking.
11. Do not add an admin override path.
12. Continue broader Phase 4/5 hardening only through bounded, reviewed tasks.
13. Continue avoiding admin override paths in other areas; no admin path should bypass evidence, ownership, sensitivity, or packet-type restrictions.
14. Design a controlled admin correction candidate classification model for parser-rule, alias/synonym, validation-rule, violation-rule, regulation/reference mapping, exception-rule, packet-template, evidence-correction, rejected, and manual-note outcomes.
15. Convert observed complex coordinate sidecar layouts into anonymized fixtures, especially unusual native PDF text ordering and scanned-PDF OCR cases that synthetic fixtures cannot fully represent.
16. Keep future internal dev plans and notes out of publicly served static assets.
17. Recheck production `dispute_packet_findings` schema-helper behavior after future helper or production DB-role changes.
