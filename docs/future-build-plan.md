...
63. Admin response-document list/detail UI now exists as a read-only visibility slice: `/admin-response-documents` is registered behind the admin route layout, appears only in admin navigation, uses existing `GET /_api/responses/list` and `GET /_api/responses/get` endpoints, and has local unit coverage at `tests/unit/response-document-ui.spec.tsx`. The UI shows response metadata filters, list/detail state, safe response metadata, packet/outcome/finding/evidence links, and evidence-only/later-report-comparison notices. It does not provide response capture, upload, review, corrected/removed/unchanged, parser, inbox, Gmail, IMAP, legal-conclusion, admin-override, or direct-furnisher controls. Staging deploy and health passed, unauthenticated response list/get APIs returned HTTP 401, and the deployed bundle contains the Response Documents page and safety text. Authenticated admin UI smoke remains pending until a safe staging admin context is configured.
...
Status: Started. Deterministic outcome comparison, persisted outcome runs/finding outcomes, compare/list/get endpoints, the admin-review endpoint, the admin Outcome Reviews UI, the response-document metadata capture backend, and the admin-only read-only Response Documents UI now exist. Authenticated staging smoke has passed for outcome tracking, outcome admin-review, admin Outcome Reviews UI, and response-document capture in both admin and user-owned contexts. Response documents remain evidence/metadata only; later deterministic report comparison is still required for corrected/removed/unchanged outcomes. Authenticated admin smoke for the Response Documents UI, consumer-facing outcome UI, response capture UI, response parser, inbox integration, response admin-review workflow, historical backfill, production-scale repeated outcome smoke coverage, and monitoring remain future work.
...
2. Run authenticated admin smoke for `/admin-response-documents` when a safe staging admin context is configured.
3. Design the consumer-facing outcome display/read-only outcome surface if desired; keep user language neutral, deterministic, evidence-linked, and non-legal-conclusive.
4. Design the response capture UI or response admin-review endpoint, depending on priority, for bureau communications including Equifax Canada email responses; keep response documents evidence/metadata only and do not treat email response as corrected or removed without later report comparison.
5. Design the response parser boundary only after capture/review workflow is stable; do not add inbox integration, change packet readiness or wording, or add an admin override.
