...
Status: Started. A no-schema deterministic comparison helper exists and is covered by synthetic unit tests, and the first persisted backend slice now records deterministic comparison runs and finding outcomes through compare/list/get endpoints. Outcome UI, admin-review endpoint, response-document workflow, historical backfill, production-scale outcome smoke coverage, and monitoring remain future work.
...
26. Service/API paths reject `active_limited_runtime`; direct SQL access must remain controlled because future statuses are reserved at the table level while service/API paths reject runtime activation.
27. No limited-runtime bridge activation exists yet.
28. No approved DB runtime bridge activation exists yet.
29. No formal runtime reference activation, rollback, version approval, or runtime bridge approval workflow exists yet.
30. No admin override path exists for regulation/reference activation.
31. No formal rule registry, rule-version approval workflow, or rollback workflow exists yet.
32. Manual-only correction replay exclusion is now covered at helper and endpoint-boundary levels, but manual-only correction classification still needs a real candidate model before broader truth-loop promotion.
33. Evidence IDs are not universal across all detector paths, even though evidence links and evidence-location metadata are improving.
34. Creditor-validation status/delete audit gaps remain.
...
2. Add external alert-delivery validation for container health, HTTP 5xx spikes, parser/OCR failures, packet generation failures, and background job/log error spikes if alerting infrastructure is available.
3. Run and record a human-observed staging restore drill into a local-only database using the existing refresh script before any broad production-scale launch.
