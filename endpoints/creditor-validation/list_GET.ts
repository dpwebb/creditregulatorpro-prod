...
import { summarizeFindingPacketReadiness } from "../../helpers/findingReadinessSummary";
...
function withReadinessMetadata<T extends {
  technicalDetails?: unknown;
  validationStatus?: string | null;
  userStatus?: string | null;
  autoGenerated?: boolean | null;
}>(row: T): T & ReturnType<typeof summarizeFindingPacketReadiness> {
  return {
    ...row,
...
export async function handle(request: Request) {
  try {
    ...
    const buildMinimalFallback = async (sourceError: unknown): Promise<OutputType> => {
      ...
      return {
        obligationTests: obligationTests.map(withReadinessMetadata) as any,
    ...
    try {
      ...
      return new Response(JSON.stringify(toJsonSafe({
        obligationTests: responseObligationTests.map(withReadinessMetadata) as any,
        total,
