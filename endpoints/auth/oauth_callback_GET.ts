  ...
  try {
    ...
    if (existingUsers.length > 0) {
      ...
      if (!existingUserAccount) {
        console.log("Backfilling userAccount for existing OAuth user:", user.id);
        await db
      ...
      // Backfill subscription row for existing users if missing
      ...
      if (!existingSubscription) {
        console.log("Backfilling subscription for existing OAuth user:", user.id);
      ...
      // Create new user (users table has no updatedAt or createdAt to set explicitly)
      ...
      // Create userAccount row for new OAuth user
      await db
        .insertInto("userAccount")
        .values({
          userId: user.id,
          email: mappedUserData.email,
          fullName: mappedUserData.displayName,
          termsAcceptedAt: null,
        })
        .execute();

      // Create subscription row for new OAuth user
      const subscriptionDefaults = await getSubscriptionDefaults(now);
      await db
        .insertInto("subscriptions")
        .values({
      ...
      // OAuth users have termsAcceptedAt = null — they will need to accept terms separately
      console.log(
        "New OAuth user created; termsAcceptedAt left null pending separate terms acceptance flow.",
      );
    }
