import { schema, OutputType, PostGridErrorType } from "./send-first-class_POST.schema";
...
export async function handle(request: Request) {
  try {
    ...
    const rateLimitResult = await checkRateLimit(
      userId.toString(),
      "SEND_FIRST_CLASS",
      RateLimitConfig.SEND_FIRST_CLASS.maxAttempts,
      RateLimitConfig.SEND_FIRST_CLASS.windowMinutes
    );
    ...
    const representationFlag = false; // CRP never represents
    ...
    if (!signature) {
      return new Response(JSON.stringify({ error: "Please add your signature in profile settings before sending." }), { status: 400 });
    }
    ...
    try {
      pgResponse = await sendRegisteredMail({
        ...
        from: fromAddress,
        pdf: dataUri,
        mailingClass: "ca_post_lettermail" // Canada Post First Class lettermail
      });
    } catch (postgridError) {
      ...
      if (!isBetaUser && input.paymentIntentId) {
        try {
          ...
          await db
            ...
            .values({
              userId,
              baseCostCad: pricing.firstClassBaseCost.toFixed(2),
              markupCad: (pricing.firstClassCost - pricing.firstClassBaseCost).toFixed(2),
              surchargeCad: "0.00",
              amountCad: pricing.firstClassCost.toFixed(2),
              description: "Canada Post First Class Mail — refunded due to dispatch failure",
    ...
    const BASE_COST_CAD = pricing.firstClassBaseCost; // raw PostGrid cost, no markup
    const MARKUP_CAD = parseFloat((pricing.firstClassCost - pricing.firstClassBaseCost).toFixed(2));
    const SURCHARGE_CAD = 0.00;
    const TOTAL_AMOUNT_CAD = pricing.firstClassCost; // total charged to user with markup
    ...
    await db.transaction().execute(async (trx) => {
      ...
      const description = `Packet sent via Canada Post First Class Mail (PostGrid ID: ${pgResponse.id})`;
      ...
      // Create postal transaction record
      await trx
        .insertInto("postalTransaction")
        .values({
          userId,
          ...
          description: "Canada Post First Class Mail — packet dispatch",
          packetId: input.packetId,
      ...
      await trx
        ...
        .values({
          ...
          details: {
            field: "delivery_info",
            method: "Canada Post First Class",
            tracking: pgResponse.trackingNumber || null,
    ...
    return new Response(JSON.stringify({
      success: true,
      message: "Packet sent via First Class mail successfully",
      trackingNumber: pgResponse.trackingNumber || null,
    ...
    console.error("Error sending packet via first class mail:", error);
