...
import { buildReportFindingDiagnosticSummary } from "../../helpers/reportFindingDiagnostics";
  ...
  try {
    ...
    if (tradelineIds.length === 0) {
      return new Response(JSON.stringify({
        ...
        diagnosticSummary: buildReportFindingDiagnosticSummary({
          artifactId: input.artifactId,
          tradelineCount: 0,
          findings: [],
          ingestionStatus: artifact.processingStatus,
          currentStep: artifact.processingStatus,
        }),
    ...
    const diagnosticSummary = buildReportFindingDiagnosticSummary({
      artifactId: input.artifactId,
      tradelineCount: tradelineIds.length,
      violationCandidateCount: filteredViolations.length,
      findings: filteredViolations.map((violation) => ({
