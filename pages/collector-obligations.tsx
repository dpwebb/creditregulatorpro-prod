...
import styles from "./collector-obligations.module.css";

export default function CollectorObligationsPage() {
  // State
  ...
  // Filter data for BILL_COLLECTOR section
  const filteredObligations = data?.obligations
    .filter((obs) => obs.section === "BILL_COLLECTOR")
    .filter((obs) => {
      const matchesSearch =
  ...
  const handleCreate = async (formData: {
    ...
    try {
      await createMutation.mutateAsync({
        ...formData,
        section: "BILL_COLLECTOR", // Enforce section
      });
      toast.success("Debt collection agency obligation created successfully");
  ...
  const handleUpdate = async (formData: {
    ...
    try {
      ...
      toast.success("Debt collection agency obligation updated successfully");
  ...
  return (
    <div className={styles.container}>
      <Helmet>
        <title>Debt Collection Agency Obligations | Credit Regulator Pro</title>
      </Helmet>

      <PageHeader
                title="Debt Collection Agency Obligations"
        subtitle="Manage regulatory requirements for debt collection agencies"
        
      >
        <Button onClick={() => setIsCreateOpen(true)}>
          <Plus size={18} />
          Add Debt Collection Agency Obligation
        </Button>
      </PageHeader>
      ...
      <div className={styles.content}>
        {isLoading ? (
          ...
          <div className={styles.emptyState}>
            <p>No debt collection agency obligations found matching your search.</p>
          </div>
        ) : (
          <TableContainer>
            <Table>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Debt Collection Agency Obligation</DialogTitle>
            <DialogDescription>
