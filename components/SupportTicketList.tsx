...
import { SupportTicketStatus, SupportTicketCategory, SupportTicketPriority } from '../helpers/schema';
import styles from './SupportTicketList.module.css';

export const formatEnum = (val?: string) => {
  if (!val) return "";
  return val.replace(/_/g, " ").replace(/\w\S*/g, w => w.charAt(0).toUpperCase() + w.substr(1).toLowerCase());
};
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.filters}>
        <Select value={filters.status || "__empty"} onValueChange={v => updateFilter({ status: v === "__empty" ? undefined : v as SupportTicketStatus })}>
          <SelectTrigger className={styles.filterSelect}><SelectValue placeholder="All Statuses" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={filters.category || "__empty"} onValueChange={v => updateFilter({ category: v === "__empty" ? undefined : v as SupportTicketCategory })}>
          <SelectTrigger className={styles.filterSelect}><SelectValue placeholder="All Categories" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={filters.priority || "__empty"} onValueChange={v => updateFilter({ priority: v === "__empty" ? undefined : v as SupportTicketPriority })}>
          <SelectTrigger className={styles.filterSelect}><SelectValue placeholder="All Priorities" /></SelectTrigger>
          <SelectContent>
        ...
        <Table>
          ...
          <TableBody>
            {isPending ? (
              ...
              <TableRow>
                <TableCell colSpan={showAgentColumn ? 6 : 5} className={styles.emptyState}>
                  No support tickets found matching your filters.
                </TableCell>
              </TableRow>
            ) : (
              data.tickets.map(ticket => (
                <TableRow key={ticket.id} onClick={() => navigate(`/support-tickets/${ticket.id}`)} className={styles.row}>
      ...
      {data && typeof (data as any).total === 'number' && (
        <div className={styles.pagination}>
          <span className={styles.paginationText}>
            Showing {data.tickets.length > 0 ? page * pageSize + 1 : 0}–{Math.min((page + 1) * pageSize, (data as any).total)} of {(data as any).total}
          </span>
          <div className={styles.paginationControls}>
            ...
            <Button 
              ...
              disabled={(page + 1) * pageSize >= (data as any).total}
            >
              Next
            </Button>
          </div>
