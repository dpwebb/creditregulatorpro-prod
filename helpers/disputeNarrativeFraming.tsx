...
export async function getDisputeLetterFraming(
  ...
  if (checkDofdSolObstruction(violationCategory, violationDetails, tradelineDetails)) {
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Missing Date of First Delinquency",
      introduction:
        "I'm writing about an account in my credit file that appears to have incomplete information. The details of my concern are explained below.",
    };
  } else if (
    violationCategory === "DOCUMENTATION_CHAIN_FAILURE" &&
    (violationDetails?.matchReason || violationDetails?.originalCreditorName)
  ) {
    framing = {
      subject:
        "Formal Dispute and Reinvestigation Request - Original Creditor and Assignment Documentation",
      introduction:
        "I'm writing to dispute the accuracy of an account listed in my credit file. My specific concern is detailed below.",
    };
  } else if (
    ...
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Possible Duplicate Collection Reporting",
      introduction:
        "I'm writing to dispute information in my credit file that appears to be reported more than once. The details are below.",
    };
  } else if (
    violationCategory &&
    [
    ...
    framing = {
      subject:
        "Formal Dispute and Reinvestigation Request - Reporting Period and Account Chronology",
      introduction:
        "I'm writing to request the removal of outdated information from my credit file. The specific account and the basis for my request are detailed below.",
    };
  } else if (violationCategory === "BANKRUPTCY_DISCHARGE_VIOLATION") {
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Bankruptcy or Insolvency Status",
      introduction:
        "I am writing regarding an account in my credit file that was included in a bankruptcy discharge but continues to be reported inaccurately.",
    };
  } else if (violationCategory === "IDENTITY_THEFT_VIOLATION") {
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Identity Theft or Unauthorized Account",
      introduction:
        "I am writing to report that the following account in my credit file is the result of identity theft and was not opened or authorized by me.",
    };
  } else if (violationCategory === "MIXED_FILE_PERSONAL_INFO_MISMATCH") {
    framing = {
      subject:
        "Formal Dispute and Reinvestigation Request - Mixed File or Personal Information Mismatch",
      introduction:
        "I'm writing because I suspect my credit file has been mixed with another consumer's information. The details are below.",
    };
  } else if (violationCategory === "COLLECTION_LIMITATION_EXCEEDED") {
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Collection Reporting Period",
      introduction:
        "I'm writing to request the removal of a collection account from my credit file. The details are below.",
    };
  } else if (violationCategory === "CONSENT_WITHDRAWAL_NOT_HONORED") {
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Consent and Reporting Basis",
      introduction:
        "I'm writing regarding an account that continues to be reported on my credit file inappropriately. The details are below.",
    };
  } else if (violationCategory === "FREEZE_PERIOD_VIOLATION") {
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Security Freeze File Access",
      introduction: "I'm writing to report unauthorized activity on my credit file. The details are below.",
    };
  } else if (
    violationCategory &&
    ["DOCUMENTATION_CHAIN_FAILURE", "METRO2_FIELD_VIOLATION", "DOFD_REPORTING"].includes(
      violationCategory
    )
  ) {
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Incomplete Reporting",
      introduction:
        "I'm writing because I found incomplete information in my credit file. The details are below.",
    };
  } else {
    const bureauStr = bureauName ? `${bureauName} ` : "";
    framing = {
      subject: "Formal Dispute and Reinvestigation Request - Accuracy and Completeness",
...
export function buildViolationAwareAccountId(
  ...
  if (tradelineDetails) {
    ...
    if (isDuplicateCollector) {
      ...
      if (violationDetails?.otherAgencyName || violationDetails?.duplicateCreditorName) {
        lines.push(
          `Duplicate Collector: ${violationDetails.otherAgencyName || violationDetails.duplicateCreditorName}`
        );
      }
      ...
      if (checkDofdSolObstruction(violationCategory, violationDetails, tradelineDetails)) {
        ...
        violationCategory === "DOCUMENTATION_CHAIN_FAILURE" &&
        (violationDetails?.originalCreditorName || violationDetails?.matchReason)
      ) {
  ...
  return enrichAccountIdentification(lines.join("\n"), {
    violationCategory,
    violationDetails,
    tradelineDetails,
  });
...
export function deduplicateLetterSections(introduction: string, disputedItems: string): string {
  ...
  const extractSentences = (text: string) => {
    ...
    return matches ? matches.map((sentence) => sentence.trim()).filter((sentence) => sentence.length > 0) : [text.trim()];
  };
  ...
  const disputedWordsSets = disputedSentences.map((sentence) => new Set(getWords(sentence)));
  ...
  const cleanedIntro = introSentences.filter((introSentence) => {
    ...
    for (const disputedWords of disputedWordsSets) {
      if (disputedWords.size === 0) continue;

      let overlapCount = 0;
      for (const word of introWords) {
        if (disputedWords.has(word)) overlapCount++;
      }
