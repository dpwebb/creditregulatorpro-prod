...
export const COMPLIANCE_DETECTOR_MANIFEST = [
  ...
  detector("detectDebtValidationFailure", "collection", "COLLECTOR", [...ACCOUNT_FIELDS, "balance", "status"], "strong", ["DOCUMENTATION_CHAIN_FAILURE"], "manual_review_prone", "Detects collector accounts with missing validation support."),
  detector("detectOriginalCreditorChainFailure", "collection", "COLLECTOR", [...ACCOUNT_FIELDS, "balance"], "strong", ["DOCUMENTATION_CHAIN_FAILURE"], "manual_review_prone", "Detects collection accounts missing clear original-creditor support."),
  ...
  detector("detectClosedAccountBalanceInflation", "balance", "CREDITOR", [...ACCOUNT_FIELDS, ...BALANCE_FIELDS, "dateClosed"], "strong", ["CLOSED_ACCOUNT_BALANCE_INFLATION"], "evidence_critical", "Detects closed accounts with unsupported balance increases after closure."),
  detector("detectZombieDebtResurrection", "collection", "CREDITOR", [...ACCOUNT_FIELDS, ...DATE_FIELDS, "balance"], "strong", ["ZOMBIE_DEBT_RESURRECTION"], "evidence_critical", "Flags old or inactive accounts that reappear after prior removal or resolution."),
  ...
  detector("detectIdentityTheftViolation", "identity", "CREDITOR", [...ACCOUNT_FIELDS, "remarks", "status"], "strong", ["IDENTITY_THEFT_VIOLATION"], "manual_review_prone", "Flags accounts with identity or authorization review indicators."),
  ...
  detector("detectConsumerStatementSuppression", "bureau", "BUREAU", [...ACCOUNT_FIELDS, "remarks", "disputeStatus"], "strong", ["CONSUMER_STATEMENT_SUPPRESSION"], "manual_review_prone", "Detects missing consumer dispute statements or alerts."),
  ...
  detector("detectFurnisherPostDisputeRetaliation", "furnisher", "CREDITOR", [...ACCOUNT_FIELDS, "status", ...RESPONSE_CONTEXT_FIELDS, ...REPORT_CONTEXT_FIELDS], "strong", ["FURNISHER_POST_DISPUTE_RETALIATION"], "manual_review_prone", "Detects adverse furnisher reporting changes after dispute activity."),
