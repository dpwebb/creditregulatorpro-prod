...
export async function persistViolations(
  ...
  const preservedViolationsAll = await db
  ...
  const activeGeneratedViolations = await db
    .selectFrom("creditorObligationTest")
    ...
    .where("userStatus", "=", "active")
    .execute();
  const deletableViolationIds = options.sourceReportArtifactId
    ? activeGeneratedViolations
        .filter((row) => violationMatchesSourceArtifact(row, options.sourceReportArtifactId!))
        .map((row) => row.id)
    : activeGeneratedViolations.map((row) => row.id);
