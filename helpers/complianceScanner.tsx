...
export async function scanForViolations(
  ...
  try {
    ...
    violations.forEach(v => {
      ...
      if (!v.technicalDetails.sourceReportArtifactId && context.sourceReportArtifactId) {
        v.technicalDetails.sourceReportArtifactId = context.sourceReportArtifactId;
      }
...
function violationArtifactScopeIds(details: unknown): number[] {
  const record = technicalDetailsRecord(details);
  if (!record) return [];
  const ids = [
    record.sourceReportArtifactId,
...
function violationMatchesSourceArtifact(
  row: { technicalDetails: unknown },
  sourceReportArtifactId: number,
): boolean {
  const scopeIds = violationArtifactScopeIds(row.technicalDetails);
...
export async function persistViolations(
  ...
  return db.transaction().execute(async (trx) => {
    const tradelineForWorkflow = await trx
      .selectFrom("tradeline")
    ...
    const preservedViolationsAll = await trx
      .selectFrom("creditorObligationTest")
      .select(["violationCategory", "userExplanation", "technicalDetails"])
      .where("tradelineId", "=", tradelineId)
      .where("userStatus", "!=", "active")
      .execute();
    const preservedViolations = options.sourceReportArtifactId
      ? preservedViolationsAll.filter((row) =>
          violationMatchesSourceArtifact(row, options.sourceReportArtifactId!),
        )
      : preservedViolationsAll;
    ...
    const activeGeneratedViolations = await trx
      .selectFrom("creditorObligationTest")
      .select(["id", "technicalDetails"])
      .where("tradelineId", "=", tradelineId)
      .where("autoGenerated", "=", true)
      .where("userStatus", "=", "active")
      .execute();
    const deletableViolationIds = activeGeneratedViolations.map((row) => row.id);
    const deleteResult = deletableViolationIds.length > 0
      ? await trx
          .deleteFrom("creditorObligationTest")
          .where("id", "in", deletableViolationIds)
          .execute()
      : [{ numDeletedRows: 0 }];
    ...
    // 2. Insert all violations from the fresh scan before committing the active-set swap.
    for (const violation of normalizedViolations) {
      ...
      const baseDetails = JSON.parse(JSON.stringify({
        ...
        ...(options.sourceReportArtifactId && !violation.technicalDetails?.sourceReportArtifactId
          ...
          : {}),
        complianceFindingReplacement: {
          mode: "atomic_active_auto_replacement",
          version: 1,
          sourceReportArtifactId: options.sourceReportArtifactId ?? null,
        },
      ...
      if (!result?.id) {
        throw new Error(`Failed to persist replacement compliance finding for ${violation.violationCategory}.`);
      }
...
export async function scanAndPersistViolations(
  ...
  const insertedIds = await persistViolations(violations, tradelineId, {
    sourceReportArtifactId: context.sourceReportArtifactId,
  });
