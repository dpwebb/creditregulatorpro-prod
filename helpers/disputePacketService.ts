...
const CANONICAL_DISPUTE_INTENT_SET = new Set<string>(CANONICAL_DISPUTE_INTENTS);

function canonicalIntentText(value: string | null | undefined): string | null {
  const normalized = typeof value === "string" ? value.trim().toUpperCase() : "";
  return CANONICAL_DISPUTE_INTENT_SET.has(normalized) ? normalized : null;
}
...
function adjudicationDetailsFrom(details: Record<string, unknown>): Record<string, unknown> | null {
...
function hasPacketAdjudicationMetadata(value: unknown): boolean {
  return Boolean(adjudicationDetailsFrom(parseDetails(value)));
}

function consumerIssueIntentFromDetails(
  row: PacketConsumerDisputedItemSource,
  details: Record<string, unknown>,
  packetType?: DisputePacketType,
): string | null {
  const adjudication = adjudicationDetailsFrom(details);
  if (!adjudication) return null;

  const adjudicatedText = firstKnownText([
    firstText(adjudication, ["consumerDisputeIntent", "consumerLabel", "findingKind"]),
    firstText(details, ["consumerDisputeIntent", "consumerLabel", "findingKind"]),
  ]);
  const canonical = canonicalIntentText(adjudicatedText);
  if (canonical) return canonical;

  return canonicalDisputeIntentFor({
    issueType: adjudicatedText,
    disputedField: fieldFromDetails(details),
    packetType,
    accountNumberMissing: !row.accountNumber,
...
function issueTypeForRow(
  ...
  return consumerIssueIntentFromDetails(row, details, packetType) ?? "reporting_issue";
}
...
function packetTypesForRow(row: IssueRow): DisputePacketType[] {
  ...
  const issueType = issueTypeForRow(row, parseDetails(row.issueTechnicalDetails), "credit_bureau").toUpperCase();
...
export function evaluatePacketReadinessForIssues(
  ...
  for (const issue of selectedIssues) {
    ...
    const missingAdjudication = issue.autoGenerated === true && !hasPacketAdjudicationMetadata(issue.technicalDetails);
    ...
    if (missingAdjudication) {
      ...
      addBlocker(
        blockers,
        "FINDING_ADJUDICATION_REQUIRED",
        "This auto-detected finding must be adjudicated before packet creation.",
        issue.issueId,
      );
    }
...
function candidateFromRow(row: IssueRow, packetType: DisputePacketType = "credit_bureau"): DisputePacketCandidate {
  ...
  return {
    ...
    issueType: labelizeIssueType(issueTypeForRow(row, details, packetType)),
    explanation: redactSensitiveText(
      sanitizeComplianceNeutralText(row.issueUserExplanation) ??
        "Review this account information against the source report.",
      row.accountNumber,
    ),
    evidenceReference,
    requestedAction: actionForIssue(issueTypeForRow(row, details, packetType), packetType, {
...
function consumerDisputeReasonForRow(
  ...
  const intent = canonicalDisputeIntentFor({
    issueType: issueTypeForRow(row, details, packetType),
    disputedField: fieldFromDetails(details),
    packetType,
    isCollectionAccount: row.isCollectionAccount,
  });
...
export function buildConsumerDisputedItemInput(
  ...
  const issueType = issueTypeForRow(row, details, packetType);
  const evidenceReference = evidenceReferenceForRow(row, details);
