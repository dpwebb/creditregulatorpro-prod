...
describe("packet readiness evaluation", () => {
  ...
  it("keeps auto-detected findings with meaningful evidence packet eligible", () => {
    const readiness = evaluatePacketReadinessForIssues(
      user,
      { packetType: "credit_bureau", selectedIssueIds: [10] },
      [
  ...
  it("blocks auto-detected findings until adjudication metadata is attached", () => {
    const readiness = evaluatePacketReadinessForIssues(
      user,
      { packetType: "credit_bureau", selectedIssueIds: [10] },
      [
    ...
    expect(readiness.reasonCodes).toContain("FINDING_ADJUDICATION_REQUIRED");
    expect(readiness.blockers).toContainEqual(expect.objectContaining({
      findingId: 10,
      code: "FINDING_ADJUDICATION_REQUIRED",
      message: "This auto-detected finding must be adjudicated before packet creation.",
    }));
  });

  it("preserves auto-detected findings without meaningful evidence but keeps them out of packets", () => {
    const readiness = evaluatePacketReadinessForIssues(
      user,
      { packetType: "credit_bureau", selectedIssueIds: [10] },
      [
        issue({
          ...
          evidenceReference: "Source report #7; field: balance",
        }),
      ],
    );

    expect(readiness.packetReady).toBe(false);
    expect(readiness.eligibleFindingIds).toEqual([]);
    expect(readiness.ineligibleFindingIds).toEqual([10]);
    expect(readiness.reasonCodes).toContain("NEEDS_USER_REVIEW");
    expect(readiness.blockers).toContainEqual(expect.objectContaining({
      findingId: 10,
      code: "NEEDS_USER_REVIEW",
      message: "This finding must be verified before packet creation.",
    }));
  });

  it("keeps verified auto-detected findings blocked when evidence metadata is still weak", () => {
    const readiness = evaluatePacketReadinessForIssues(
      user,
      { packetType: "credit_bureau", selectedIssueIds: [10] },
      [
