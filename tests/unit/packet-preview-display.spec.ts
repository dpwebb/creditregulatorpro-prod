...
describe("packet preview display content", () => {
  it("builds recipient-facing preview text from the humanized letter path", () => {
    ...
    expect(combined).toContain("Account: Account number not shown on report");
    ...
    expect(pdfLetterText).toContain("Account: Account number not shown on report");
    expect(pdfLetterText).toContain("Information disputed: Date last reported");
    expect(pdfLetterText).toContain("Reported value: Aug 21, 2012");
    expect(pdfLetterText).toContain("Requested result: Verify the correct information");
    expect(pdfLetterText).toContain("Requested action:");
