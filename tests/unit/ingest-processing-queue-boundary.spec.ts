...
describe("ingest processing queue boundaries", () => {
  ...
  it("proves deploy and compose wiring has a bounded ingest worker path", () => {
    ...
    expect(productionCompose).not.toContain("creditregulatorpro-ingest-worker");
    expect(productionWorkflow).toContain("explicit-bounded-production-ingest-worker-apply");
    expect(productionWorkflow).toContain("--source authenticated_ingest_process");
    expect(productionWorkflow).toContain("--concurrency 1");
    ...
    expect(productionWorkflow).toContain("docker compose -f docker-compose.production.yml up -d --build creditregulatorpro");
    expect(productionWorkflow).not.toContain("creditregulatorpro creditregulatorpro-ingest-worker");
